...
export class WhatsAppController {
  async handleIncoming(
    ...
    try {
      logger.info(`[WhatsApp] Controller reached`);
      ...
      logger.info(
        "[WhatsApp] Calling whatsappService.handleIncomingMessage...",
      );
      const twimlReply = await whatsappService.handleIncomingMessage(
        from,
        body,
      );
